...
{
    # Check glibc-static
    if(!( -f '/usr/lib64/libm.a' ) && !( -f '/usr/lib/libm.a') && !( -f '/usr/lib/x86_64-linux-gnu/libm.a'))
    {
...
#    }
    ...
    $https   .= `find /usr/lib/ -name SSLeay.so -print`;
    $https =~ s/\s//g;
    if(!length($https))
    {
    ...
    $https .= `find /usr/lib/      -name JSON    -print`;
    $https .= `find /usr/local/share/perl/5.26.1 -name JSON.pm -print`;
    $https  =~ s/\s//g;
    if(!length($https))
    {
